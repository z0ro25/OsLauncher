...
import com.truongnt.ios.launcher.compat.ShortcutConfigActivityInfo;
import com.truongnt.ios.launcher.compat.UserHandleCompat;
import com.truongnt.ios.launcher.compat.UserManagerCompat;
import com.truongnt.ios.launcher.graphics.LauncherIcons;
import com.truongnt.ios.launcher.graphics.ShadowGenerator;
import com.truongnt.ios.launcher.model.WidgetItem;
...
import com.truongnt.ios.launcher.widget.IWidgetPreview;
import com.truongnt.ios.launcher.widget.WidgetCell;
...
public class WidgetPreviewLoader {
    ...
    public PreviewLoadRequest getPreview(final Object o, int previewWidth,
                                         int previewHeight, IWidgetPreview caller) {
    ...
    public WidgetCacheKey getObjectKey(Object o, String size) {
        ...
        if (o instanceof LauncherAppWidgetProviderInfo) {
            ...
            return new WidgetCacheKey(item.componentName, item.user, size);
        } else {
    ...
    @Thunk
    Bitmap generatePreview(Context context, Object info, Bitmap recycle,
                           int previewWidth, int previewHeight) {
        if (info instanceof LauncherAppWidgetProviderInfo) {
            return generateWidgetPreview(context, (LauncherAppWidgetProviderInfo) info,
                    previewWidth, previewHeight, recycle, null);
        } else if (info instanceof WidgetItem) {
            WidgetItem item = (WidgetItem)info;
            return generateShortcutPreview(context, item.activityInfo,
                    previewWidth, previewHeight, recycle);
        } else {
            return generateShortcutPreview(context,
                    (ResolveInfo) info, previewWidth, previewHeight, recycle);
        }
    }

    public Bitmap generateWidgetPreview(Context context, LauncherAppWidgetProviderInfo info,
        ...
        if (info.isIOSIconWidget) {
            previewWidth = previewHeight = LauncherAppState.getIDP(context).portraitProfile.widgetPreviewImageSizePx;
        } else {
        ...
        if (previewWidth > maxPreviewWidth || previewHeight > maxPreviewHeight) {
            scale = Math.min(maxPreviewWidth / (float) (previewWidth), maxPreviewHeight / (float) (previewHeight));
        }
        ...
        if (widgetPreviewExists) {
            drawable.setBounds(x, y, x + previewWidth, y + previewHeight);
            drawable.draw(c);
        } else {
            final Paint p = new Paint();
            p.setFilterBitmap(true);
            int appIconSize = LauncherAppState.getIDP(context).portraitProfile.iconSizePx;
        ...
        int imageHeight = Math.min(preview.getHeight(), previewHeight);
        return mManager.getBadgeBitmap(info, preview, imageHeight);
    }

    private Bitmap generateShortcutPreview(
            Context context, ResolveInfo info, int maxWidth, int maxHeight, Bitmap preview) {
        final Canvas c = new Canvas();
        if (preview == null) {
...
//        icon.draw(c);
    ...
    private Bitmap generateShortcutPreview(Context context, ShortcutConfigActivityInfo info,
                                           int maxWidth, int maxHeight, Bitmap preview) {
        int iconSize = LauncherAppState.getIDP(context).portraitProfile.iconSizePx;
    ...
    public class PreviewLoadTask extends AsyncTask<Void, Void, Bitmap> {
        ...
        private final IWidgetPreview mCaller;
        @Thunk
        long[] mVersions;
        @Thunk
        ...
        protected Bitmap doInBackground(Void... params) {
            ...
            if (!isCancelled() && preview == null) {
                ...
                preview = generatePreview(mCaller.getContext(), mInfo, unusedBitmap, mPreviewWidth, mPreviewHeight);
            }
            return preview;
        }
