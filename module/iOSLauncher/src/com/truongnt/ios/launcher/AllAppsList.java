...
class AllAppsList {
    ...
    public void addPackage(Context context, String packageName, UserHandleCompat user) {
        ...
        for (LauncherActivityInfoCompat info : matches) {
            if (!IOSOtaHandler.isAppHide(context, packageName)) {
                AppInfo appInfo = new AppInfo(context, info, user, mIconCache);
                if (Partner.getBoolean(context, Partner.FEATURE_NEW_INSTALL_ICON_ENABLE, true)
                        && (appInfo.flags & AppInfo.DOWNLOADED_FLAG) != 0) {
                    appInfo.newInstalled = true;
                    NewInstallAppHandler.updateNewFlagInDatabase(context, appInfo);
                }
                add(appInfo);
            }
        }
    ...
    public void addPackageEntry(Context context, String componentName, UserHandleCompat user) {
        ComponentName addedEntry = ComponentName.unflattenFromString(componentName);
        String packageName = addedEntry.getPackageName();
        final LauncherAppsCompat launcherApps = LauncherAppsCompat.getInstance(context);
        final List<LauncherActivityInfoCompat> matches = launcherApps.getActivityList(packageName,
                user);

        for (LauncherActivityInfoCompat info : matches) {
            if (info.getComponentName().getClassName().equals(addedEntry.getClassName())) {
                add(new AppInfo(context, info, user, mIconCache));
            }
        }
    }

    public void removePackageEntry(String componentName, UserHandleCompat user) {
        final List<AppInfo> data = this.data;
        ComponentName removedEntry = ComponentName.unflattenFromString(componentName);
        for (int i = data.size() - 1; i >= 0; i--) {
            AppInfo info = data.get(i);
            final ComponentName component = info.intent.getComponent();
            if (info.user.equals(user) && component.equals(removedEntry)) {
                removed.add(info);
                data.remove(i);
            }
        }
    ...
    public void updatePackageFlags(StringFilter pkgFilter, UserHandleCompat user, FlagOp op) {
        ...
        for (int i = data.size() - 1; i >= 0; i--) {
            ...
            if (info.user.equals(user) && pkgFilter.matches(component.getPackageName())) {
                //Log.e("zwb","zwb ----- updatePackageFlags = " + component.getPackageName());
    ...
    public void updatePackage(Context context, String packageName, UserHandleCompat user) {
        ...
        if (matches.size() > 0) {
            ...
            for (final LauncherActivityInfoCompat info : matches) {
                ...
                if ((!IOSOtaHandler.isAppHide(context, packageName)) &&
                        (!IOSOtaHandler.isAppEntryHide(context, packageName, info.getComponentName().getClassName()))) {
