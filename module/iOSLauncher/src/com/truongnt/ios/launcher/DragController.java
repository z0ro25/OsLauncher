...
public class DragController {
    ...
    private ArrayList<DragListener> mPopupListeners = new ArrayList<DragListener>();
    ...
    private float downX = 0, upX = 0, downY = 0, upY = 0;
    ...
    public void startDrag(View v, Bitmap bmp, DragSource source, Object dragInfo,
        ...
        startDrag(bmp, dragLayerX, dragLayerY, source, dragInfo, dragAction, null,
                null, initialDragViewScale, false, null);
    ...
    public DragView startDrag(Bitmap b, int dragLayerX, int dragLayerY,
                              ...
                              float initialDragViewScale, boolean accessible, DragOptions options) {
        ...
        // Drag có thể được khởi động bằng code (long-press → showInfo) mà không đi qua
        // ACTION_DOWN của onTouchEvent, khiến downX còn giá trị cũ/0. Đặt lại baseline
        // theo điểm chạm ban đầu để ACTION_MOVE đo khoảng cách từ đúng vị trí ngón tay,
        // tránh đóng menu nổi ngay lập tức khi mới chỉ giữ (chưa kéo).
        downX = mMotionDownX;
        downY = mMotionDownY;
    ...
    private void callOnDragStart() {
        for (DragListener listener : mPopupListeners) {
            listener.onDragStart(mDragObject.dragSource, mDragObject.dragInfo, mDragAction);
        }
    ...
    private void callOnDragEnd() {
        ...
        for (DragListener listener : new ArrayList<>(mPopupListeners)) {
            listener.onDragEnd();
        }
    }
    ...
    public Bitmap getViewBitmap(View v) {
        ...
        if (v instanceof BubbleTextView) {
            flag = ((BubbleTextView) v).getTextVisibility();
        ...
        if (v instanceof BubbleTextView) {
            ((BubbleTextView) v).setTextVisibility(flag);
        }
    ...
    public void onAppsRemoved(ItemInfoMatcher matcher) {
        // Cancel the current drag if we are removing an app that we are dragging
        if (mDragObject != null) {
            if (mDragObject.dragInfo instanceof ShortcutInfo) {
                ItemInfo dragInfo = (ShortcutInfo) mDragObject.dragInfo;
    ...
    private void endDrag() {
        ...
        mLauncher.getWorkspace().forceLayout();
     ...
     */
    public int[] getClampedDragLayerPos(float x, float y) {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        /*if (mLauncher.isOpeningFloatingMenu()) {
            Log.d(TAG, "closeFloatingMenu: onTouchEvent");
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                Log.d(TAG, "onTouchEventxxx ACTION_DOWN" + ev.getX());
                break;
            case MotionEvent.ACTION_MOVE:
                // Khi bắt đầu kéo, đóng menu nổi để nó không nuốt sự kiện kéo
                if (Math.abs(ev.getX() - downX) > ViewConfiguration.get(mLauncher).getScaledTouchSlop()) {
                    if (mLauncher.isOpeningFloatingMenu()) {
                        mLauncher.closeFloatingMenu();
                    }
                }
                ...
                break;
            case MotionEvent.ACTION_UP:
                Log.d(TAG, "onTouchEventxxx ACTION_UP" + ev.getX());
    ...
    private DropTarget findDropTarget(int x, int y, int[] dropCoordinates) {
        ...
        for (int i = (count - 1); i >= 0; i--) {
            ...
            if (r.contains(x, y)) {
                ...
                Log.d("findDropTarget", "findDropTarget = rect" + r + "x,y=" + x + "," + y);
                return target;
            }
        }

        Log.d("findDropTarget", "findDropTarget = false");
        return null;
    }
    ...
    public void addPopupDragListener(DragListener l) {
        mPopupListeners.add(l);
    }

    public void removePopupDragListener(DragListener l) {
        mPopupListeners.remove(l);
    }
