...
import com.truongnt.ios.launcher.graphics.WorkspaceAndHotseatScrim;
import com.truongnt.ios.launcher.util.MultiValueAlpha;
...
public class DragLayer extends InsettableFrameLayout {
    ...
    public DragLayer(Context context, AttributeSet attrs) {
        ...
        this.mDockGesDistance = res.getInteger(R.integer.ios_dockbar_gesture_distance);
        this.mStatusBarHeight = (int) res.getDimension(R.dimen.status_bar_height);
    }
    ...
    public void setup(Launcher launcher, DragController controller) {
        ...
        mScrim.setWorkspace(launcher.getWorkspace());
    }
    ...
    private boolean isEventOverFolderTextRegion(Folder folder, MotionEvent ev) {
        ...
        boolean isContain = mHitRect.contains((int) ev.getX(), (int) ev.getY());
        return isContain;
    }
...
//        getDescendantRectRelativeToSelf(mLauncher.getSearchDropTargetBar(), mHitRect);
    ...
    private boolean handleTouchDown(MotionEvent ev, boolean intercept) {
        ...
        if (currentFolder != null && intercept) {
            if (currentFolder.isEditingName() && !isEventOverFolder(currentFolder, ev)) {
                currentFolder.dismissEditingName();
                return true;
            }  else if (isEventOverFolderTextRegion(currentFolder, ev)) {
                return false;
            } else {
        ...
        if (this.mLauncher.isInWorkspace() && this.mLauncher.isWorkspaceNormal() && !this.mLauncher.isInPreviewView()) {
            ...
            switch (motionEvent.getAction() & 255) {
                ...
                case MotionEvent.ACTION_UP:
                    int offsetX = (int) (x - this.mInterceptXDown);
                    int offsetY = (int) (y - this.mInterceptYDown);
                    ...
                    if (Math.abs(yVelocity) > ((float) this.mMinimumFlingVelocity) &&
                            Math.abs(offsetY) > this.mDockGesDistance &&
                            Math.abs(offsetY) > Math.abs(offsetX) * 2 &&
                            this.mInterceptYDown < ((float) getMeasuredHeight()) &&
                            this.mInterceptYDown > ((float) this.mStatusBarHeight)) {
                        ...
                        if (cellLayout != null) {
                            ...
                            for (int i3 = 0; i3 < childCount; i3++) {
                                ...
                                if (childAt instanceof AppWidgetHostView) {
                                    Rect rect = new Rect();
                                    ((AppWidgetHostView) childAt).getGlobalVisibleRect(rect);
                                    if (this.mInterceptYDown > ((float) rect.top) && this.mInterceptYDown < ((float) rect.bottom)) {
                                        return false;
                                    }
                                }
                            }
                        ...
                        if (offsetY < 0) {
                            GestureEventModel.triggerGestureEvent(mLauncher, GestureEventModel.GESTURE_SWIPE_UP, GestureEventModel.DIRECTION_UP);
                        } else {
                            GestureEventModel.triggerGestureEvent(mLauncher, GestureEventModel.GESTURE_SWIPE_DOWN, GestureEventModel.DIRECTION_DOWN);
                        }
                        return true;
                    }
...
//                this.mIsOnTouching = false;
    ...
    public void animateViewIntoPosition(DragView dragView, final View child, int duration,
        ...
        if (child instanceof CustomTextView) {
            CustomTextView tv = (CustomTextView) child;
    ...
    public void explodeView(final DragView view,final Rect from){
        mLauncher.getDeleteExplosionField().explode(view,from);
    }
    ...
    public boolean isOpenFloatingMenu() {
        int childCount = getChildCount();
        for (int i = 0; i < childCount; i++) {
            if (getChildAt(i) instanceof AppWidgetResizeFrame) {
                return true;
            }
        }
        return false;
