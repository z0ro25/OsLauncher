...
public class WorkspaceStateTransitionAnimation {
    ...
    @Thunk
    float mWorkspaceRootViewShrinkFactor;
    @Thunk
    ...
    public WorkspaceStateTransitionAnimation(Launcher launcher, Workspace workspace) {
        ...
        mWorkspaceRootViewShrinkFactor =
                res.getInteger(R.integer.config_workspaceRootViewShrinkPercentage) / 100f;
    ...
    public void setOverviewModeShrinkFactor(float factor) {
        mOverviewModeShrinkFactor = factor;
    }
    ...
    private void animateWorkspace(final TransitionStates states, int toPage, final boolean animated,
        ...
        float finalHotseatAlpha = (states.stateIsNormal || states.stateIsSpringLoaded) ?
                1f : 0f;
        float finalWallpaperBgAlpha = states.stateIsOverview ? 1f : 0f;
        ...
        float finalWorkspacePgIndicatorAlpha = (states.stateIsNormal || states.stateIsSpringLoaded || states.stateIsOverview) ?
                1f : 0f;
        float finalWorkspacePgIndicatorTranslationY = states.stateIsOverview || states.stateIsOverviewHidden ?
                mWorkspace.getOverviewModePgIndicatorTranslationY() : 0;

        float finalWorkspaceRootViewScaleFactor = states.stateIsNormalHidden ? mWorkspaceRootViewShrinkFactor : 1.0f;
        ...
        if (!states.stateIsNormal) {
            if (states.stateIsSpringLoaded) {
                ...
                mWorkspace.setPivotY(mWorkspace.getMeasuredHeight() / 2);
                mWorkspace.setPivotX(mWorkspace.getMeasuredWidth() / 2);
            } else if (states.stateIsOverview || states.stateIsOverviewHidden) {
                mNewScale = mOverviewModeShrinkFactor;
                mWorkspace.setPivotY(mWorkspace.getChildTopOffset());
                mWorkspace.setPivotX(mWorkspace.getMeasuredWidth() / 2);
            }
        }
...
//        mWorkspace.snapToPage(toPage, duration, mZoomInInterpolator);
        ...
        final WorkspaceRootView workspaceRootView = mLauncher.getWorkspaceRootView();
        final View wallpaperBackground = mLauncher.getWallpaperBackgroud();
        final View hotseat = mLauncher.getHotseat();
        final View pageIndicator = mWorkspace.getPageIndicator();
        if (animated) {
            LauncherViewPropertyAnimator scaleRootView = new LauncherViewPropertyAnimator(workspaceRootView);
            scaleRootView.scaleX(finalWorkspaceRootViewScaleFactor)
                    .scaleY(finalWorkspaceRootViewScaleFactor)
                    .setDuration(duration)
                    .setInterpolator(mZoomInInterpolator);
            //mStateAnimator.play(scaleRootView);
            ...
            if (pageIndicator != null) {
                pageIndicatorAlpha = new LauncherViewPropertyAnimator(pageIndicator)
                        .alpha(finalWorkspacePgIndicatorAlpha)
                        .translationY(finalWorkspacePgIndicatorTranslationY)
                        .withLayer();
            ...
            LauncherViewPropertyAnimator wallpaperBgAlpha =
                    new LauncherViewPropertyAnimator(wallpaperBackground).alpha(finalWallpaperBgAlpha);
            wallpaperBgAlpha.addListener(new AlphaUpdateListener(wallpaperBackground,
                    accessibilityEnabled));

            // For animation optimations, we may need to provide the Launcher transition
            // with a set of views on which to force build layers in certain scenarios.
            hotseat.setLayerType(View.LAYER_TYPE_HARDWARE, null);
            wallpaperBackground.setLayerType(View.LAYER_TYPE_HARDWARE, null);
            if (layerViews != null) {
                // If layerViews is not null, we add these views, and indicate that
                // the caller can manage layer state.
                layerViews.put(hotseat, LauncherStateTransitionAnimation.BUILD_AND_SET_LAYER);
                layerViews.put(wallpaperBackground, LauncherStateTransitionAnimation.BUILD_AND_SET_LAYER);
            } else {
                // Otherwise let the animator handle layer management.
                hotseatAlpha.withLayer();
                wallpaperBgAlpha.withLayer();
            }

            wallpaperBgAlpha.setDuration(duration);
            ...
            workspaceRootView.setScaleX(finalWorkspaceRootViewScaleFactor);
            workspaceRootView.setScaleY(finalWorkspaceRootViewScaleFactor);
            wallpaperBackground.setAlpha(finalWallpaperBgAlpha);
            AlphaUpdateListener.updateVisibility(wallpaperBackground, accessibilityEnabled);
            hotseat.setAlpha(finalHotseatAlpha);
            AlphaUpdateListener.updateVisibility(hotseat, accessibilityEnabled);
            if (pageIndicator != null) {
                pageIndicator.setAlpha(finalWorkspacePgIndicatorAlpha);
                pageIndicator.setTranslationY(finalWorkspacePgIndicatorTranslationY);
    ...
    private void animateBackgroundGradient(TransitionStates states,
        ...
        float finalAlpha = states.stateIsNormal || states.stateIsOverview ? 0 : mWorkspaceScrimAlpha;
