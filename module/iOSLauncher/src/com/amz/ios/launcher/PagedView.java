...
import com.amz.ios.launcher.effect.ScrollEffect;
import com.amz.ios.launcher.touch.OverScroll;
...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
    ...
    private int[] mPageScrolls = new int[999];
    ...
    protected PageIndicator.PageMarkerResources getPageIndicatorMarker(int pageIndex) {
        return new PageIndicator.PageMarkerResources(PageIndicator.PAGEDVIEW_WORKSPACE);
    }
    ...
    public void scrollTo(int x, int y) {
        ...
        if (isLoopEnable()) {
            ...
            boolean isNeedForceSetX = (x >= mMaxScrollX);
            /*
            if (isNeedForceSetX) {
                super.scrollTo(x, y);
            } else {
             ...
             */
                ...
                if (isXBeforeFirstPage) {
                    ...
                    if (mAllowOverScroll) {
                         ...
                         */
                            scrollToPrevFirst(x % getViewportWidth());
                    }
                    ...
                    if (mAllowOverScroll) {
                         ...
                         */
                            scrollToAfterEnd(x % getViewportWidth());
                    }
...
//        }
        ...
        for (int i = 0; i < 999; i ++) {
            int nOffset = mPageScrolls[i];
            if (nOffset <= 0 && i >= 2) {
                mPageScrolls[i] = mPageScrolls[i-1] + mPageScrolls[i-1] - mPageScrolls[i-2];
            }
        }
            ...
            if (child != null) {
                ...
                switch (LauncherInteractor.INSTANCE.getMPageAnimationType()){
                    ...
                    case ZOOM_IN:
                        ...
                        if (!endScroll) {
                            float zoomScale = Math.max(0.8f, 1 + Math.abs(scrollProgress) * 0.5f);
                            child.setScaleX(zoomScale);
                            child.setScaleY(zoomScale);
                        } else {
                            child.setAlpha(1f);
                            resetViewPropertyValues(child);
                        }
                    ...
                    default:
                        boolean endScrolls = Math.abs(scrollProgress) == 1.0f;
                        if (useTransitionEffect() && mScrollEffect != null && !endScrolls) {
                            mScrollEffect.screenScrolled(child, i, scrollProgress);
                        } else {
                            child.setAlpha(1f);
                            resetViewPropertyValues(child);
                        }
    ...
    public void setScrollEffectFromString(String effectName) {
        ScrollEffect.setFromString(this, effectName);
    }
    ...
    public void setScrollEffect(ScrollEffect effect) {
    ...
    protected float getScrollProgress(int screenCenter, View v, int page) {
        ...
        if (isLoopEnable()) {
            int offset = count * getViewportWidth();
            if (delta > mMaxScrollX) {
//                delta = delta - offset;
            }
            else if (delta < -mMaxScrollX) {
//                delta = delta + offset;
            }
        }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                int movePointerIndex = ev.findPointerIndex(mActivePointerId);

                if (movePointerIndex == -1) return true;

                float moveX = ev.getX(movePointerIndex);
                float moveDeltaX = mLastMotionX + mLastMotionXRemainder - moveX;

                if (mTouchState == TOUCH_STATE_SCROLLING) {
                    // Scroll to follow the motion event

                    mTotalMotionX += Math.abs(moveDeltaX);

                    // Only scroll and update mLastMotionX if we have moved some discrete amount.  We
                    // keep the remainder because we are actually testing if we've moved from the last
                    // scrolled position (which is discrete).
                    if (Math.abs(moveDeltaX) >= 1.0f) {
                        mTouchX += moveDeltaX;
                        mSmoothingTime = System.nanoTime() / NANOTIME_DIV;

                        scrollBy((int) moveDeltaX, 0);
                        mLastMotionX = moveX;
                        mLastMotionXRemainder = moveDeltaX - (int) moveDeltaX;
                    } else {
                        awakenScrollBars();
                    }
            ...
            case MotionEvent.ACTION_UP:
                if (mTouchState == TOUCH_STATE_SCROLLING) {
                    ...
                    if (((isSignificantMove && !isDeltaXLeft && !isFling) ||
                        ...
                        if (isDeltaXLeft) {
                            if ((!isSignificantMove || isFling) && !(isFling && isVelocityXLeft)) {
                                closeRightPage();
                            } else {
                                openRightPage();
                            }
                        } else if ((!isSignificantMove || isFling) && (!isFling || isVelocityXLeft)) {
                            closeLeftPage();
                        } else {
                            openLeftPage();
                        }
    ...
    protected void snapToPageWithVelocity(int whichPage, int velocity) {
        ...
        if (mTouchState == TOUCH_STATE_NEXT_PAGE && mCurrentPage == getPageCount()) return;
        if (mTouchState == TOUCH_STATE_PREV_PAGE && mCurrentPage == 0) return;
