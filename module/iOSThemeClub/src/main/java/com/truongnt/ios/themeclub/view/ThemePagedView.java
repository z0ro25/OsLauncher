...
package com.truongnt.ios.themeclub.view;
...
import com.truongnt.ios.ioslite.common.util.Utilities;
import com.truongnt.ios.themeclub.R;
import com.truongnt.ios.launcher.PageIndicator;
import com.truongnt.ios.themeclub.MineThemeScroller;
import com.truongnt.ios.themeclub.effect.ThemeScrollEffect;
import com.truongnt.ios.themeclub.touch.OverScroll;
import com.truongnt.ios.themeclub.util.Thunk;
 ...
 */
public class ThemePagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
    ...
    private static final float OVERSCROLL_DAMP_FACTOR = 0.25f;

    private static final float RETURN_TO_ORIGINAL_PAGE_THRESHOLD = 0.33f;
    // The page is moved more than halfway, automatically move to the next page on touch up.
    private static float SIGNIFICANT_MOVE_THRESHOLD = 0.4f;
    ...
    protected ThemeScrollEffect mScrollEffect;
    ...
    protected MineThemeScroller mScroller;
...
//    protected ScrollEffect mScrollEffect;
    ...
    ArrayList<Integer> mScreenOrder = new ArrayList<Integer>();
    ...
    public ThemePagedView(Context context) {
        this(context, null);
        mContext = context;
    }
    ...
    public ThemePagedView(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs,
                com.truongnt.ios.launcher.R.styleable.PagedView, defStyle, 0);

        mPageLayoutWidthGap = a.getDimensionPixelSize(
                com.truongnt.ios.launcher.R.styleable.PagedView_pageLayoutWidthGap, 0);
        mPageLayoutHeightGap = a.getDimensionPixelSize(
                com.truongnt.ios.launcher.R.styleable.PagedView_pageLayoutHeightGap, 0);
        mPageIndicatorViewId = a.getResourceId(com.truongnt.ios.launcher.R.styleable.PagedView_pageIndicator, -1);
        a.recycle();
     ...
     */
    protected void init() {
        String launcherSignificantMoveThreshold = Partner.getString(getContext(), Partner.DEF_LAUNCHER_SIGNIFICANT_MOVE_THRESHOLD);
        if (!TextUtils.isEmpty(launcherSignificantMoveThreshold)) {
            try {
                SIGNIFICANT_MOVE_THRESHOLD = Float.valueOf(launcherSignificantMoveThreshold);
            }catch (Exception e){
                e.printStackTrace();
            }
        }
        mScroller = new MineThemeScroller(getContext());
...
//            mPageIndicator = (PageIndicator) findViewById(mPageIndicatorViewId);
    ...
    int getChildTopOffset() {
        return mChildTopOffset;
    }
...
//            if (count > 1) {
//                return new PageIndicator.PageMarkerResources(R.drawable.ic_theme_pageindicator_current,
//                        R.drawable.theme_pageindicator_add);
//            }
//        }
//
//        return getPageIndicatorMarker(pageIndex);
        return new PageIndicator.PageMarkerResources(PageIndicator.PAGEDVIEW_THEMESPACE);
    }
    ...
    public void setOnPageChangeListener(OnPageChangeListener pageChangeListener) {
        ...
        if (pageChangeListener != null) {
            mOnPageChangeListener.onPageSelected(getCurrentPage());
        }
    }
    ...
    private int validateNewPage(int newPage) {
        ...
        if (isLoopEnable()) {
            validatedPage = (newPage + getPageCount()) % getPageCount();
    ...
    protected void onPageBeginMoving() {
        if (mPageSwitchListener != null) {
            mPageSwitchListener.onPageBeginMoving();
        }
    }
    ...
    protected void onPageEndMoving() {
        mWasInOverscroll = false;
        if (mPageSwitchListener != null) {
            mPageSwitchListener.onPageEndMoving();
        }
    }
...
//        return Settings.sLoopEnable;
    ...
    public void setViews(final List<Bitmap> views, int type){
        removeAllViews();
        mViewList = views;
        for (int i = 0; i < views.size(); i++) {
            RelativeLayout layout = (RelativeLayout) mInflater.inflate(
                    R.layout.theme_detail_view, this, false /* attachToRoot */);
            ImageView imageView = (ImageView) layout.findViewById(R.id.themeclubImageview);

            if (type == 0) {//theme flow page for apply
                int screenWidth = getResources().getDisplayMetrics().widthPixels;
                final RelativeLayout.LayoutParams layoutParams = new RelativeLayout.LayoutParams(screenWidth * 3 / 4, ViewGroup.LayoutParams.WRAP_CONTENT);
                imageView.setLayoutParams(layoutParams);
            }
            else{ //theme flow page for show
            ...
            final Bitmap bitmap = views.get(i);
            ...
            imageView.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    if (m_themeImageListener != null)
                        m_themeImageListener.image(finalI);
                }
            });
    ...
    protected void screenScrolled(int screenCenter) {
        ...
        for (int i = numCustomPages(); i < getChildCount(); i++) {
            ...
            if (v != null) {
                ...
                if (useTransitionEffect() && mScrollEffect != null && !endScroll) {
                    mScrollEffect.screenScrolled(v, i, scrollProgress);
                } else {
                    v.setAlpha(1f);
                    resetViewPropertyValues(v);
                }
            }
    ...
    private void resetViewPropertyValues(View v) {
    ...
    public void setScrollEffectFromString(String effectName) {
        ThemeScrollEffect.setFromString(this, effectName);
    }
    ...
    public void setScrollEffect(ThemeScrollEffect effect) {
        mScrollEffect = effect;
    ...
    public void showScrollEffectAnimation() {
        if (!mScroller.isFinished()) {
            Log.d(TAG, "showScrollEffectAnimation isPlaying or Scroller not finished.");
            return;
        }

        int pageCount = getChildCount() - numCustomPages();
        if (pageCount >= 2) {
            final int originalPage = mCurrentPage;
            int destPage = mCurrentPage == numCustomPages() ? numCustomPages() + 1 : mCurrentPage - 1;
            snapToPage(destPage, PAGE_SNAP_ANIMATION_DURATION);
            postDelayed(new Runnable() {
                @Override
                public void run() {
                    snapToPage(originalPage, PAGE_SNAP_ANIMATION_DURATION);
                }
            }, 800);
        }
    }
...
//        return hasCustomContent() ? 1 : 0;
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (pageCount > 0) {
            ...
            if (mOnPageChangeListener != null) {
                float scrollProgress = getScrollProgress(screenCenter, getPageAt(leftScreen), leftScreen);
                mOnPageChangeListener.onPageScrolled(leftScreen, scrollProgress);
            }
            ...
            case MotionEvent.ACTION_UP:
                if (mTouchState == TOUCH_STATE_SCROLLING) {
                    ...
                    if (((isSignificantMove && !isDeltaXLeft && !isFling) ||
                            (isFling && !isVelocityXLeft)) && (mCurrentPage > 0 || isLoopEnable())) {
                        finalPage = returnToOriginalPage ? mCurrentPage : mCurrentPage - 1;
                        snapToPageWithVelocity(finalPage, velocityX);
                    } else if (((isSignificantMove && isDeltaXLeft && !isFling) ||
                            (isFling && isVelocityXLeft)) &&
                            (mCurrentPage < getChildCount() - 1 || isLoopEnable())) {
                        finalPage = returnToOriginalPage ? mCurrentPage : mCurrentPage + 1;
                        snapToPageWithVelocity(finalPage, velocityX);
                    } else {
                        snapToDestination();
                    ...
                    if (isLoopEnable()) {
                        nextPage = (mCurrentPage - 1 + getChildCount()) % getChildCount();
                    }
                    ...
                    if (isLoopEnable()) {
                        nextPage = (mCurrentPage + 1) % getChildCount();
                    }
...
//        (LocalThemeDetailActivity)mContext).onClick(this);
    }
    ...
    protected void snapToPage(int whichPage, int delta, int duration, boolean immediate,
        ...
        int startX = getUnboundedScrollX();

        if (isLoopEnable()) {
            int lastPage = getChildCount() - 1;
            int viewportWidth = getViewportWidth();
            if (oldWhichPage < 0 && whichPage == lastPage) {
                delta = -(startX + viewportWidth);
                startX += mMaxScrollX + viewportWidth;
            } else if (oldWhichPage > lastPage && whichPage == 0) {
                delta = mMaxScrollX + viewportWidth - startX;
                startX = -delta;
            }
        }
...
//                    onPostReorderingAnimationCompleted();
//                    if (mDragView instanceof CellLayout) {
//                        CellLayout cellLayout = (CellLayout) mDragView;
//                        cellLayout.transiteBackground(false);
//                    }
//
//                }
//            });
...
//            if (mDragView instanceof CellLayout) {
//                CellLayout cellLayout = (CellLayout) mDragView;
//                cellLayout.transiteBackground(true);
//            }
//            mDragViewBaselineLeft = mDragView.getLeft();
...
//        }
    ...
    protected String getCurrentPageDescription() {
        return String.format(getContext().getString(R.string.themeclub_default_scroll_format),
    ...
    private ThemeImageListener m_themeImageListener;
    public void setOnThemeImageListener(ThemeImageListener listener){
        m_themeImageListener = listener;
    }
    public interface ThemeImageListener{
        void image(int currentIdx);
    }
