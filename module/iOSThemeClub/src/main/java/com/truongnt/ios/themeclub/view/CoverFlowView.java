...
import com.truongnt.ios.themeclub.adapter.CoverFlowAdapter;
import com.truongnt.ios.themeclub.util.BitmapUtils;
...
public class CoverFlowView<T extends CoverFlowAdapter> extends View {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = startPos; i < visibleCount + startPos; ++i) {
            ...
            maxChildTotalHeight = (maxChildTotalHeight < childTotalHeight) ? childTotalHeight
                    : maxChildTotalHeight;
            maxChildTotalWidth = (maxChildTotalWidth < childWidth) ? childWidth
                    : maxChildTotalWidth;
        }
    ...
    protected final void drawChild(Canvas canvas, int mid, int position, float offset) {
        ...
        if (child != null && !child.isRecycled() && canvas != null) {
            ...
            mChildTransformer.setRectToRect(new RectF(0, 0, child.getWidth(), child.getHeight()), new RectF(0, 0, mWidth, mChildHeight), Matrix.ScaleToFit.FILL);
            canvas.drawBitmap(child, mChildTransformer, mDrawChildPaint);
//            canvas.drawBitmap(child, 0, 0, mDrawChildPaint);
            if (reflection != null) {
