...
package com.truongnt.ios.ioslite.common.debug;
...
public class MemoryTracker extends Service {
    ...
    public static final String ACTION_START_TRACKING = "com.ios.ioslite.action.START_TRACKING";
    ...
    public void startTrackingProcess(int pid, String name, long start) {
        name = formatProcessName(this, name);
        synchronized (mLock) {
        ...
        synchronized (mLock) {
            ...
            for (int i = 0; i < dinfos.length; i++) {
                ...
                if (BuildUtil.ATLEAST_MARSHMALLOW) {
                    info.uss[info.head] = info.currentUss = Long.valueOf(dinfo.getMemoryStat("summary.java-heap"));
                } else {
                    info.uss[info.head] = info.currentUss = dinfo.dalvikPrivateDirty;
                }
    ...
    private static String formatProcessName(Context context, String orginName) {
        String packageName = context.getPackageName();
        if (orginName.equals(packageName)) {
            return "Main";
        } else if (orginName.contains(":")) {
            return orginName.split(":")[1];
        }
        return "A";
    }
}
